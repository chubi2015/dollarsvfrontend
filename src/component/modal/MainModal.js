...
const MainModal = ({ modalOpen, setModalOpen, children, mobile }) => {
  ...
  return (
    <>
      <Transition appear show={modalOpen} as={Fragment}>
        <Dialog
          as="div"
          className={`${mobile ? 'fixed inset-0 flex items-center justify-center' : 'fixed inset-0 z-30 overflow-y-auto text-center'}`}
          onClose={() => setModalOpen(false)}
          initialFocus={cancelButtonRef}
        >
          <div className="min-h-screen px-4 inset-0">
            <Transition.Child
              as={Fragment}
